...
let AppComponent = class AppComponent {
    ...
    openDialog() {
        ...
        dialogRef.afterClosed().subscribe(result => this.postService.save(result.post, result.file));
    }
};
...
let PostService = class PostService {
    constructor(http) {
        ...
        this.http.get("/api/").subscribe((posts) => {
            ...
            for (let p of posts) {
                this.posts.push(new _post__WEBPACK_IMPORTED_MODULE_3__["Post"](p.name, p.title, p.subtitle, p.email, p.message, p.filePath, p.id, p.likes));
            }
        });
    }
    save(post, file) {
...
class Post {
    constructor(name, title, subtitle, email, message, filePath, id, likes) {
        this.name = name;
        this.title = title;
